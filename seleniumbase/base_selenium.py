...
class SeleniumBase:
    ...
    def input_textbox(self, locator, text, clear_existing=True) -> WebElement:
        """
    ...
    def select_radio_button(self, locator) -> WebElement:
        """
    ...
    def search_and_select_dropdown_option(self, locator, search_term) -> WebElement:
        ...
        dropdown.click()
        dropdown_input_locator = (By.XPATH, "//input[@type='search']")
        dropdown_input = self.find_element(dropdown_input_locator)
    
        dropdown_input.send_keys(search_term)
    
        # Wait for dropdown options to appear
        dropdown_option_locator = (By.XPATH, "//div[@role='option']")
        WebDriverWait(self.driver, 10).until(EC.visibility_of_any_elements_located(dropdown_option_locator))
    
        # Select the first option, you may want to adjust this based on your specific requirements
